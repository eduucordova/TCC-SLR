class ScidirsUsersProtocol < ActiveRecord::Base

  # user_hash = {user_protocol_id, percentage}
  def self.randomize_studies (user_hash, protocol)
    studies = Scidir.where('protocol_id = ?', protocol.id)
    total = studies.count()
    ...
    while stopped != user_array.count
      ...
      if !user_array[user_index][0].nil?
        if user_array[user_index][1].zero?
          ...
          ScidirsUsersProtocol.create(:users_protocol_id => user_array[user_index][0], :scopu_id => studies[study_index].id)
          study_index += 1
        end
      end
      user_index += 1
