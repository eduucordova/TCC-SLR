class AcmsUsersProtocol < ActiveRecord::Base
  ...
  def self.randomize_studies (user_hash, protocol)
    studies = Acm.where('protocol_id = ?', protocol.id)
    AcmsUsersProtocol.where(acm_id: studies).delete_all
    total = studies.count()
    ...
    while stopped != user_array.count
      ...
      if !user_array[user_index][0].nil?
        if user_array[user_index][1].zero?
          ...
          while AcmsUsersProtocol.exists?(:users_protocol_id => user_array[user_index][0], :acm_id => studies[study_index].id)
            study_index += 1
            study_index = study_index.modulo(studies.count)
          end
          AcmsUsersProtocol.create(:users_protocol_id => user_array[user_index][0], :acm_id => studies[study_index].id)
          study_index += 1
        end
      end
      user_index += 1
